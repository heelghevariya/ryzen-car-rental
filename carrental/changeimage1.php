...
{
	$result=mysqli_query($conn,"update tbl_vehicles set vimage1='".$vimage1."' where vehicle_id=$id") or die(mysqli_error($conn));
...
<?php 
$id=intval($_GET['imgid']);
$sql=mysqli_query($conn,"SELECT vimage1 from tbl_vehicles where tbl_vehicles.vehicle_id=$id") or die(mysqli_error($conn));
$cnt=1;
